...
from src.inference.model_vlm import MiniMindVLM, VLMConfig
from trainer_utils import setup_seed
warnings.filterwarnings('ignore')

def init_model(args):
    tokenizer = AutoTokenizer.from_pretrained(args.load_from, trust_remote_code=True)
    if args.load_from == './model/MiniMind2-V':
        ...
        model = MiniMindVLM(
            ...
            vision_model_path="./model/vision_model/chinese-clip-vit-base-patch16"
        )
        state_dict = torch.load(ckp, map_location=args.device, weights_only=False)
        model.load_state_dict({k: v for k, v in state_dict.items() if 'mask' not in k}, strict=False)
    else:
        model = AutoModelForCausalLM.from_pretrained(args.load_from, trust_remote_code=True)
        model.vision_encoder, model.processor = MiniMindVLM.get_vision_model("./model/vision_model/chinese-clip-vit-base-patch16")
    
    print(f'VLM模型参数: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} M(illion)')
    preprocess = model.processor
    # [New Feature] Dynamic Token ID Update
    # Update image_ids based on actual tokenization of the special token to ensure robustness
    if hasattr(model, 'params') and hasattr(model.params, 'image_special_token'):
        ids = tokenizer.encode(model.params.image_special_token, add_special_tokens=False)
        model.params.image_ids = ids
        
        # Validation: Check if token count matches the number of image patches (usually 196)
        # 196 is the sequence length of ViT-B/16 (224/16)**2 + 1(cls) ... typically just patch count 196.
        # Actually MiniMind-V config might expect 196.
        if len(ids) != 196:
            print(f"[Warning] Tokenized image placeholder length ({len(ids)}) does not match expected length (196). This might cause alignment issues if the model expects 196 tokens.")
...
def main():
    ...
    parser.add_argument('--load_from', default='./model/MiniMind2-V', type=str, help="模型加载路径（model=原生torch权重，其他路径=transformers格式）")
    parser.add_argument('--save_dir', default='./model/MiniMind2-V', type=str, help="模型权重目录")
    ...
    for image_file in sorted(os.listdir(args.image_dir)):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            # Memory Cleanup
            del inputs, pixel_values, messages, inputs_text
